...
export default function AddRecipeModal({ familySlug, editRecipeId }: { familySlug?: string; editRecipeId?: number }) {
  ...
  return (
    <>
      {isEdit ? (
        <button className="btn btn-primary btn-3d btn-sm" onClick={() => setOpen(true)}>Edit</button>
      ) : (
        <button className="btn btn-primary btn-3d" onClick={() => setOpen(true)}>+ Add a Recipe</button>
      )}

      {open && (
        <dialog className="modal modal-open">
          <div className="modal-box rounded-2xl">
            <h3 className="font-bold text-lg mb-4">{isEdit ? 'Edit Recipe' : 'Add a Recipe'}</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
                ...
                <div className="space-y-2">
                  {fields.map((f, idx) => (
                    <div key={f.id} className="flex gap-2">
                      <input className="input input-bordered w-full" {...register(`ingredients.${idx}` as const)} />
              ...
              <div className="form-control">
                ...
                <textarea className="textarea textarea-bordered h-32" {...register('instructions')} />
                {errors.instructions && <span className="text-error text-sm">{String((errors.instructions as any).message)}</span>}
              </div>
              <div className="form-control">
                <label className="label">Image (jpg/png/webp; max 5MB)</label>
                <input type="file" accept="image/*" className="file-input file-input-bordered" {...register('image')} />
              </div>
